...
const AikotobaEdit: React.FC<Props> = (props) => {
  // 入力フォームのエラーチェック用
  ...
  // エラーメッセージ
  const _errorMessage = `任意の文字列を ${props.min}〜${props.max} 文字以内で入力してください。`
  // 項目チェック OK メッセージ
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // OK メッセージ表示チェック
    if (props.min <= props.text.length) {
      setIsOk(true)
    } else {
      setIsOk(false)
    }
  ...
  const handleBlur = () => {
    ...
    setVisible(false)
    // ture の時エラーとみなされる
    setIsError(!(props.min <= props.text.length))
  }
